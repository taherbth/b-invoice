...
function checkCreditCard($card_info, &$errornumber, &$errortext) {

$cardnumber = $card_info['credit_card_no'];
$cardname = $card_info['credit_card_type'];
  ...
  $ccErrors [5] = "Verification code wrong";
  $ccErrors [6] = "Card Is Expired";
  ...
  // Verify Card Verification Code(cvv2)
    $cvv2 = $card_info['credit_card_verification_code'];
    if($cardname=="Amex"){
...
///Check Card Expire date
    $card_expire_date_month   = $card_info['card_expire_date_month'];
    $card_expire_date_year      = $card_info['card_expire_date_year'];
    $expiry_date = mktime(0, 0, 0, ($card_expire_date_month + 1), 1, $card_expire_date_year);
    /*echo date("Y-m-d",$expiry_date);
