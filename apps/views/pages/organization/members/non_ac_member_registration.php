...
<h3><?php echo $this->lang->line('org_admin_non_ac_member_registration_msg');?></h3>
     ...
     if($create_new_member){ 
       if($customer_type == $org_type){
        ...
        $customer_no= array(
              ...
              'value'     => $customer_no,          
			  'size'       =>"30",
              'onblur'        => "check_customer_no_availability(this.value)"        
            );
        ...
        /*$dept_or_company_or_org_name = array(
		 ...
		 $mobile_phone = array(
              ...
              'value'     => $mobile_phone,          
              'size'       =>"30",
              'onblur'        => "check_mobile_availability(this.value)"      
             );
        ...
        $vat_no= array(
              ...
              'value'     => $vat_no,  
              'size'       =>"30",
              'onblur'        => "check_vat_no_availability(this.value)"        
			  );       
        $web= array(
              'name'      => 'web',
              'id'        => 'web',
        ...
        $your_reference= array(
         ...
         $ean_no= array(
              ...
              'value'     => $ean_no,  
              'size'       =>"30",
              'onblur'        => "check_ean_no_availability(this.value)"        
...
<script>
 ...
 var controller = 'info';
  ...
  function check_ean_no_availability(ean_no){    
                $.ajax({
                    'url' : base_url + '/organization/' + controller + '/ean_no_availability/'+ean_no,
                    'type' : 'POST', //the way you want to send data to your URL
                    'data' : {'type' : 'nnn'},
                    'success' : function(data){ //probably this request will return anything, it'll be put in var "data"
                        var container = $('#ean_no_availability_status'); //jquery selector (get element by id)
                        if(data){
                            container.html(data);
                        }
                    }
  ...
  function check_vat_no_availability(vat_no){    
                $.ajax({
                    'url' : base_url + '/organization/' + controller + '/vat_no_availability/'+vat_no,
                    'type' : 'POST', //the way you want to send data to your URL
                    'data' : {'type' : 'nnn'},
                    'success' : function(data){ //probably this request will return anything, it'll be put in var "data"
                        var container = $('#vat_no_availability_status'); //jquery selector (get element by id)
                        if(data){
                            container.html(data);
                        }
                    }
  ...
  function check_mobile_availability(mobile_no){  
                $.ajax({
                    'url' : base_url + '/organization/' + controller + '/mobile_no_availability/'+mobile_no,
                    'type' : 'POST', //the way you want to send data to your URL
                    'data' : {'type' : 'nnn'},
                    'success' : function(data){ //probably this request will return anything, it'll be put in var "data"
                        var container = $('#mobile_no_availability_status'); //jquery selector (get element by id)
                        if(data){
                            container.html(data);
                        }
                    }
 ...
 function check_customer_no_availability(customer_no){    
                $.ajax({
                    'url' : base_url + '/organization/' + controller + '/customer_no_availability/'+customer_no,
                    'type' : 'POST', //the way you want to send data to your URL
                    'data' : {'type' : 'nnn'},
                    'success' : function(data){ //probably this request will return anything, it'll be put in var "data"
                        var container = $('#customer_no_availability_status'); //jquery selector (get element by id)
                        if(data){
                            container.html(data);
                        }
                    }
...
<?php echo form_open("organization/info/create_new_non_ac_member_process"); ?>
...
<div class="infobox" style="width: 900px; margin-bottom: 20px; margin-left:10px; font-size: 12px; -moz-border-radius: 8px 8px 8px 8px;">
    ...
    <table width="662" cellspacing="1" style="" align="center" cellpadding="2" border="0" id="theBody">
        ...
        <tr>
                <td width="38%"></td>
                <td width="33%"> <span class="markcolor"><?php echo ucwords($label_all_address_empty_err_msg); ?></span>  </td>
        </tr>
        ...
        <tr>
            ...
            <div id="dept_org_company_name" >
               ...
               <tr>
                <td width="38%"><font class="inside"><?php echo $this->lang->line('label_customer_no'); ?>:</font></td>
                <td width="33%"><?=form_input($customer_no);?><span class="markcolor"><?php echo ucwords(form_error('customer_no')); ?></span> 
                </td>
            </tr>          
              <tr>
                <td width="38%">
                <td width="33%">
                    <span id="customer_no_availability_status"></span> 
                </td>
            </tr>
             <tr>
            ...
            <tr>
                <td width="38%"><font class="inside"><?php echo $this->lang->line('label_billing_email'); ?>:</font></td>
            ...
            <tr>
                <td width="38%"><font class="inside"><?php echo $this->lang->line('label_billing_mobile'); ?>:</font></td>
              ...
              <tr>
                <td width="38%">
                <td width="33%">
                    <span id="mobile_no_availability_status"></span> 
                </td>
            </tr>
            <tr>
                <td width="38%"><font class="inside"><?php echo $this->lang->line('label_phone_or_fax'); ?>:</font></td>
                <td width="33%"><?=form_input($land_line_phone);?><sspan class="markcolor"><?php echo ucwords(form_error('land_line_phone')); ?></span> 
                </td>
            </tr>
            <tr>
                <td width="38%"><font class="inside"><?php echo $this->lang->line('label_vat_no'); ?>:</font></td>
                <td width="33%"><?=form_input($vat_no);?><sspan class="markcolor"><?php echo ucwords(form_error('vat_no')); ?></span> 
                </td>
            </tr>
            <tr>
                <td width="38%">
                <td width="33%">
                    <span id="vat_no_availability_status"></span> 
                </td>
            </tr>
             <tr>
                <td width="38%"><font class="inside"><?php echo $this->lang->line('label_ean_no'); ?>:</font></td>
                <td width="33%"><?=form_input($ean_no);?><sspan class="markcolor"><?php echo ucwords(form_error('ean_no')); ?></span> 
                </td>
            </tr>
            <tr>
                <td width="38%">
                <td width="33%">
                    <span id="ean_no_availability_status"></span> 
                </td>
            </tr>
            <tr>
                <td width="38%"><font class="inside"><?php echo $this->lang->line('label_web'); ?>:</font></td>
                <td width="33%"><?=form_input($web);?><sspan class="markcolor"><?php echo ucwords(form_error('web')); ?></span> 
                </td>
            </tr>
            <tr>
                <td width="38%"><font class="inside"><?php echo $this->lang->line('label_attention'); ?>:</font></td>
                <td width="33%"><?=form_input($attention);?><sspan class="markcolor"><?php echo ucwords(form_error('attention')); ?></span> 
                </td>
            </tr>
            <tr>
                <td width="38%"><font class="inside"><?php echo $this->lang->line('label_your_reference'); ?>:</font></td>
                <td width="33%"><?=form_input($your_reference);?><sspan class="markcolor"><?php echo ucwords(form_error('your_reference')); ?></span> 
                </td>
            </tr>
            <tr>
                <td width="38%"><font class="inside"><?php echo $this->lang->line('label_our_reference'); ?>:</font></td>
                <td width="33%"><?=form_input($our_reference);?><sspan class="markcolor"><?php echo ucwords(form_error('our_reference')); ?></span> 
                </td>
            </tr>
           
            <tr>
                <td width="38%"><font class="inside"><?php echo $this->lang->line('label_credit_limit'); ?>:</font></td>
                <td width="33%"><?=form_input($credit_limit);?><sspan class="markcolor"><?php echo ucwords(form_error('credit_limit')); ?></span> 
                </td>
            </tr>
            <tr>
                <td width="38%"><font class="inside"><?php echo $this->lang->line('label_discount')."(%)"; ?>:</font></td>
                <td width="33%"><?=form_input($discount);?><sspan class="markcolor"><?php echo ucwords(form_error('discount')); ?></span> 
                </td>
            </tr>
          </div>  
