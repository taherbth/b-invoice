...
<tr>
	<td style="width:100px" valign="top" style="padding-top: 12px; font-weight:bold"><b><?php echo $this->lang->line('label_sent_to');?> :</b></td>
	<td>
		<?php if($send_to_individual_email){ echo $send_to_individual_email; }
                    if($send_to_member){ 
                        $receivers_mem_id = ltrim ($send_to_member,',');
                        $receivers_mem_id = rtrim ($receivers_mem_id,',');
                        $receivers_member_info = $this->info_model->get_member_info_by_id_for_communication($receivers_mem_id);
                        if($receivers_member_info){
                            foreach($receivers_member_info as $receivers_rows){
                                if($receivers_rows->non_ac_member){
                                    echo $receivers_rows->attention."(".$this->lang->line('label_non_acm').") | ";
                                }else{echo $receivers_rows->first_name.$receivers_rows->last_name." | ";}
                            }
                        }
                    }
