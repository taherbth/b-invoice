...
<?php
       ...
       if($customer_type == $organization_data['org_type']){
...
function checkemail_availability(email_val){    
                $.ajax({
                    'url' : base_url + '/admin/' + controller + '/email_availability/'+email_val,
                ...
                $.ajax({
                    'url' : base_url + '/admin/' + controller + '/username_availability/'+email_val,
                    'type' : 'POST', //the way you want to send data to your URL
                    'data' : {'type' : 'nnn'},
                    'success' : function(data){ //probably this request will return anything, it'll be put in var "data"
...
function check_username_availability(username){                        
                $.ajax({
                    'url' : base_url + '/admin/' + controller + '/username_availability/'+username,
                    'type' : 'POST', //the way you want to send data to your URL
                    'data' : {'type' : 'nnn'},
                    'success' : function(data){ //probably this request will return anything, it'll be put in var "data"
                        var container = $('#availability_status_username'); //jquery selector (get element by id)
...
<div class="infobox" style="width: 900px; margin-bottom: 20px; margin-left:10px; font-size: 12px; -moz-border-radius: 8px 8px 8px 8px;">
    ...
    <table width="662" cellspacing="1" style="" align="center" cellpadding="2" border="0" id="theBody">
        ...
        <tr>
                <td width="38%"><font class="inside"><b><?php echo $this->lang->line('label_admin_user_address_details'); ?>: </b></font></td>
    ...
    <table width="662" style="margin-top:10px" cellspacing="1" align="center" cellpadding="2" border="0" id="theBody">
        <tbody>
            <tr>  
                    <td><input id="org_type_hid" type="hidden" value=<?php echo $organization_data['org_type'];?> name="org_type_hidden"></td>
                    <td><input id="org_name_hid" type="hidden" value=<?php echo $organization_data['org_name'];?> name="org_name_hidden"></td>
                    <td><input id="org_no_hid" type="hidden" value=<?php echo $organization_data['org_number'];?> name="org_no_hidden"></td>
                    <td><?=form_hidden('organization_data', $organization_data);?></td>
                    <td><?=form_hidden('data_category', $category_name);?></td>
                    <td><?=form_hidden('package_id', $package_id);?></td>
                    <td><?=form_hidden('basic_package_selection', $basic_package_selection);?></td>
                </td>
            </tr>
            <tr>
