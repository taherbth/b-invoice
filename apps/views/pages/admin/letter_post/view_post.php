...
foreach ($r->result() as $l):
    $org_id = $l->org_id;
    $letter_id = $l->id;
    $query0 = $this->db->query("select *  from organization_info where id ='" . $org_id . "'");
    foreach ($query0->result() as $letter_info1):
        $organization_name = $letter_info1->org_name;
        $org_number = $letter_info1->org_number;
    endforeach;
	    if($l->sender_name != ""):
        $query85 = $this->db->query("select * from member where mem_id='" . $l->sender_name . "'");
        foreach ($query85->result() as $m_name):
            $name = $m_name->first_name. "&nbsp;". $m_name->last_name;
        endforeach;
		endif;
        ?>
