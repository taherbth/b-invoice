...
$receiver_ids = $letter_data['receiver_ids'];
$sql = "SELECT * FROM member mem, organization_info org_info 
             WHERE (org_info.approval_status=1) 
             AND (mem.mem_id IN($receiver_ids) AND mem.org_id=org_info.id)";
$result = mysql_query($sql);

if(mysql_num_rows($result)){
while ($rows = mysql_fetch_array($result)) {

if($rows['country']=="DEU"){$rows['country']= "GERMAN";}
if($rows['country']=="NOR"){$rows['country'] = "NORWAY";}
if($rows['country']=="DNK"){$rows['country'] = "DENMARK";}
if($rows['country']=="FIN"){$rows['country'] = "FINLAND";}
if($rows['country']=="GBR"){$rows['country'] = "UK";}
if($rows['country']=="SWE"){$rows['country'] = "SWEDEN";}
...
<tr>
    <td valign="top" style="width:50%;text-align:left; padding-left:20px;">		
        <b>From : </b><br />
      '.$rows['org_name'].'('.$rows['org_number'].')<br/>
    ...
    <td style="width:50%;text-align:left; padding-left:20px;">
        <b>To :</b><br />
        '.$rows['first_name'].'&nbsp;'.$rows['last_name'].'<br/>
         '.$rows['primary_address'].
                '<br /> '.$rows['city'].'<br /> '.$rows['zip'].' &nbsp; '.$rows['state']
                .' <br /> '.$rows['country'].' <br /> Tel &nbsp;'.$rows['phone'].'
        <br />        
...
$mpdf->Output('org_member_letter/letter_address/address_'.$letter_data['letter_title'].'_'.$letter_id.'.pdf','F');
