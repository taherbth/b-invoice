...
class DX_Auth {
    ...
    function _init() {
        ...
        $this->user_home_uri = $this->ci->config->item('DX_user_home_uri');
    ...
    function _set_last_ip_and_last_login($user_id) {
        ...
        if (!empty($data)) {
            ...
            $this->ci->load->model('admin/users', 'users');
    ...
    function _increase_login_attempt() {
        ...
        if ($this->ci->config->item('DX_count_login_attempts') AND !$this->is_max_login_attempts_exceeded()) {
            ...
            $this->ci->load->model('admin/login_attempts', 'login_attempts');
    ...
    function _get_role_data($role_id) {
        ...
        $this->ci->load->model('admin/roles', 'roles');

        $this->ci->load->model('admin/permissions', 'permissions');
    ...
    function _create_autologin($user_id) {
        ...
        $this->ci->load->model('admin/user_autologin', 'user_autologin');
    ...
    function get_emp_id() {
        return $this->ci->session->userdata('DX_emp_id');
    }

    // Get Employee id

    function get_dept_name() {
        return $this->ci->session->userdata('DX_dept_name');
    }
    ...
    // Check is user has user privilege

    function is_user() {

        return strtolower($this->ci->session->userdata('DX_role_name')) == 'user';
    }
    ...
    function is_username_available($username) {
        // Load Models
        $this->ci->load->model('admin/users', 'users');
        $this->ci->load->model('admin/user_temp', 'user_temp');
...
// Check if username is available for admin user, by making sure there is no same username in the database

    function is_admin_username_available($username,$id) {
        // Load Models
        $this->ci->load->model('admin/users', 'users');
        //$this->ci->load->model('admin/user_temp', 'user_temp');
        $users = $this->ci->users->check_admin_username($username,$id);
        //$temp = $this->ci->user_temp->check_admin_username($username);
    ...
    function is_email_available1($email) {  // Load Models    
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_email1($email);
        return $users->num_rows() == 0;
}

  function is_fak_invoice_no_available($fak_invoice_no) {  // Load Models    
        $this->ci->load->model('organization/info_model', 'info_model');
        $faktura_invoice = $this->ci->info_model->check_fak_invoice_no($fak_invoice_no);
        return $faktura_invoice->num_rows() == 0;
}

  function is_customer_no_available($customer_no, $org_id) {  // Load Models    
        $this->ci->load->model('organization/info_model', 'info_model');
        $customer_no_info = $this->ci->info_model->check_customer_no($customer_no, $org_id);
        return $customer_no_info->num_rows() == 0;
}

function is_vat_no_available($vat_no, $org_id) {  // Load Models    
        $this->ci->load->model('organization/info_model', 'info_model');
        $vat_no_info = $this->ci->info_model->check_vat_no($vat_no, $org_id);
        return $vat_no_info->num_rows() == 0;
}

function is_ean_no_available($ean_no, $org_id) {  // Load Models    
        $this->ci->load->model('organization/info_model', 'info_model');
        $ean_no_info = $this->ci->info_model->check_ean_no($ean_no, $org_id);
        return $ean_no_info->num_rows() == 0;
}

  function is_mobile_no_available($mobile_phone) {  // Load Models    
        $this->ci->load->model('organization/info_model', 'info_model');
        $mobile_phone_info = $this->ci->info_model->check_mobile_no($mobile_phone);
        return $mobile_phone_info->num_rows() == 0;
}

function is_email_available_by_org_id($email,$org_id){ 
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_email_by_org_id($email,$org_id);
        return $users->num_rows() == 0;
}

function is_fak_invoice_no_available_by_org_id($fak_invoice_no,$org_id){ 
        $this->ci->load->model('organization/info_model', 'info_model');
        $faktura_invoice = $this->ci->info_model->check_fak_invoice_no_by_org_id($fak_invoice_no,$org_id);
        return $faktura_invoice->num_rows() == 0;
}

function is_email_available_by_member_id($email,$mem_id){  
        $this->ci->load->model('organization/member', 'member');
        $member = $this->ci->member->check_email_by_member_id($email,$mem_id);
        return $member->num_rows() == 0;
}

   function is_admin_email_available1($email,$id) {  // Load Models
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_admin_email1($email,$id);
        return $users->num_rows() == 0;
    }

    function is_org_name_available($org_name) {  // Load Models
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_org_name($org_name);
        return $users->num_rows() == 0;
}

  function is_org_no_available($org_no) {  // Load Models
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_org_no($org_no);
        return $users->num_rows() == 0;
    }

    function is_org_category_available($org_category, $id1) {  // Load Models
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_org_category($org_category, $id1);
        return $users->num_rows() == 0;
    }

    function is_currency_available($currency, $id1) {  // Load Models
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_currency($currency, $id1);
        return $users->num_rows() == 0;
    }

    function org_email1_available($email, $id1) {
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_mail1($email, $id1);
        return $users->num_rows() == 0;
    }

    function org_name1_available($org_name, $id1) {
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_orgname1($org_name, $id1);
        return $users->num_rows() == 0;
    }

    function login_username($username) {
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_uname($username);
        return $users->num_rows() == 0;
    }
    ...
    function login_username_check_by_org_id($username,$org_id) {
        ...
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_uname_by_org_id($username,$org_id);
        return $users->num_rows() == 0;
}
    ...
    function login_username_check_by_member_id($username,$mem_id) {
        ...
        $this->ci->load->model('organization/member', 'member');
        $member = $this->ci->member->check_uname_by_member_id($username,$mem_id);
        return $member->num_rows() == 0;
    }
    // Check if login attempts bigger than max login attempts specified in config
    function is_org_group_available($group_name, $org_id , $mem_id) {
        $this->ci->load->model('organization/info_model', 'info_model');
        $info_model = $this->ci->info_model->check_org_group($group_name, $org_id,  $mem_id);
        return $info_model->num_rows() == 0;
    }

    function is_org_group1_available($group_name, $org_id, $mem_id, $id1) {
        $this->ci->load->model('organization/info_model', 'info_model');
        $info_model = $this->ci->info_model->check_org_group1($group_name, $org_id, $mem_id , $id1);
        return $info_model->num_rows() == 0;
    }
    function u_type_available($user_type,$id1,$org_id) {
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_type($user_type,$id1,$org_id);
        return $users->num_rows() == 0;
    }
    function is_member_username_available($username) {
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_member_username($username);
        return $users->num_rows() == 0;
    }

    function is_member_email_available($email) {
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_member_email($email);
        return $users->num_rows() == 0;
    }
   function is_person_number_available($person_number){
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_person_number($person_number);
        return $users->num_rows() == 0;
   
   }
    function is_person_number_available1($person_number){
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_person_number1($person_number);
        return $users->num_rows() == 0;
}

   function is_person_number_available_by_org_id($person_number,$org_id){
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_person_number_by_org_id($person_number,$org_id);
        return $users->num_rows() == 0;
}

   function is_person_number_available_by_member_id($person_number,$mem_id){
        $this->ci->load->model('organization/member', 'member');
        $member = $this->ci->member->check_person_number_by_member_id($person_number,$mem_id);
        return $member->num_rows() == 0;
}

 function is_admin_person_number_available1($person_number,$id){
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_admin_person_number1($person_number,$id);
        return $users->num_rows() == 0;
   }
  function org_u1_available($username,$id1){
       $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_u1($username,$id1);
        return $users->num_rows() == 0;
  }
   function org_u1_available_person($person_number,$id1){
       $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_u1_person($person_number,$id1);
        return $users->num_rows() == 0;
  }
    function is_package_name_available($package_name, $id1) {
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_package($package_name, $id1);
        return $users->num_rows() == 0;
    }
   function is_letter_header_available($title, $org_id) {
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_letter_header($title, $org_id);
        return $users->num_rows() == 0;
    }
   function is_letter_header_available1($title, $id) {
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_letter_header1($title, $id);
        return $users->num_rows() == 0;
    }
  function is_letter_footer_available($title, $org_id) {
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_letter_footer($title, $org_id);
        return $users->num_rows() == 0;
    }
 function is_letter_footer_available1($title, $id) {
        $this->ci->load->model('admin/users', 'users');
        $users = $this->ci->users->check_letter_footer1($title, $id);
        return $users->num_rows() == 0;
    }
function is_article_category_available($article_category, $id1){
	 $this->ci->load->model('admin/users', 'users');
	 $users = $this->ci->users->check_article_category($article_category, $id1);
	 return $users->num_rows() == 0;
...
function check_category($article_category,$id,$org_id){
	 //$this->ci->load->model('admin/users', 'users');
     $this->ci->load->model('organization/info_model', 'info_model');
	 $info_model = $this->ci->info_model->check_category($article_category, $id,$org_id);
	 return $info_model->num_rows() == 0;
 ...
 function is_max_login_attempts_exceeded() {
    ...
    function register($user_type, $name, $username, $password, $email) {
